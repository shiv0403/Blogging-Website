...
//   Post.findOne({_id:key}, (err, foundpost) => {
//     if(!err){
//       if(foundpost){
//         console.log("Found the postID!");
    ...
    if(!err){
      if(foundpost){
        for(var i=0;i<foundpost.length;++i){
          console.log(_.lowerCase(foundpost[i].post_title));
          if(_.lowerCase(foundpost[i].post_title) === key){
            console.log("Found the post!");
            res.render("post", {post:foundpost[i]});
            break;
          }
        }
      }else{
